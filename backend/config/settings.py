...
DATABASES = {
    "default": {
        ...
        # Было не задано, то есть 0: КАЖДЫЙ запрос открывал новое соединение с
        # Postgres и закрывал его в конце. Под ASGI это особенно заметно —
        # синхронные вьюхи и presence-вызовы из консьюмера выполняются в пуле
        # потоков, то есть connect/auth/disconnect происходит на каждый чих, а
        # Postgres на каждое соединение ещё и форкает процесс. Латентность от
        # этого скачет ровно там, где её видно хуже всего — на отправке
        # сообщения.
        #
        # Верхняя граница не абстрактная: живых соединений будет примерно
        # (число потоков ASGI + фоновые sweep-потоки) на процесс, и все они
        # упираются в postgres max_connections (по умолчанию 100). Поэтому
        # число потоков задано явно — см. ASGI_THREADS в backend/entrypoint.sh.
        #
        # В тестах — строго 0. TransactionTestCase гоняет консьюмеры в
        # отдельных потоках (см. GatewayVoiceSignalingTests), и постоянные
        # соединения из этих потоков переживают тест: тестовую БД потом не
        # удалить («is being accessed by other users»), а между тестами течёт
        # чужое состояние.
        "CONN_MAX_AGE": 0 if RUNNING_TESTS else int(os.getenv("DB_CONN_MAX_AGE", "60")),
        # Django 5.0 не умеет сам проверять, не умерло ли переиспользуемое
        # соединение (health checks появились раньше, но именно с постоянными
        # соединениями это обязательная пара): без этого рестарт Postgres или
        # разрыв со стороны сети превращал КАЖДОЕ висящее соединение в
        # InterfaceError на первом же запросе после него.
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        "support": "5/min",
        # Превью ссылок (chat.views.LinkPreviewView). Единственная ручка,
        # которая заставляет НАШ сервер идти по чужому адресу, — то есть
        # готовый инструмент и для сканирования сети чужими руками, и для
        # того, чтобы завалить кого-нибудь запросами с нашего IP. Повторы
        # почти всегда приходят в кэш и сюда не доходят вовсе.
        "link_preview": "20/min",
    },
}
