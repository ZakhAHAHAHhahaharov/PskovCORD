...
PERMISSION_FIELDS = [
    ...
    ("create_expressions", "Создавать средства выражения эмоций",
     "expressions", "Позволяет добавлять на сервер свои эмодзи и стикеры."),
    ("manage_expressions", "Управление выражениями",
     "expressions", "Позволяет переименовывать и удалять эмодзи и стикеры "
                    "сервера — в том числе загруженные другими."),
...
# set(), а не {}: пустые фигурные скобки — это ПУСТОЙ СЛОВАРЬ, и каталог прав
# сравнивался бы с ним же (см. тест test_upcoming_flag_marks_features_that_do
# _not_exist_yet — он на этом и ловит).
#
# Пусто по существу: и эмодзи (chat.models.ServerEmoji), и стикеры
# (chat.models.Sticker) появились, и все права вокруг них по-настоящему
# проверяются — в chat.views (загрузка, переименование, удаление) и в
# chat.emoji (usable_ids / usable_sticker_ids). Множество оставлено: звуковая
# доска и прочие «средства выражения» ещё впереди, и когда под них заведут
# право заранее — ему сюда.
UPCOMING_PERMISSIONS = set()
